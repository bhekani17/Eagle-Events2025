...
// Route imports (alphabetical order)
 ...
 * Logs all incoming requests with method, URL, status code, response time, and IP
 */
app.use((req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    const logMessage = `[${new Date().toISOString()}] ${method} ${originalUrl} ${statusCode} - ${responseTime}ms - ${ip}`;
